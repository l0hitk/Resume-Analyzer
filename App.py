...
nlp = spacy.load("en_core_web_sm", prefer_gpu=False)
...
def insert_data(name, email, res_score, no_of_pages, cand_level, skills):
    if name is None or name.strip() == '':
        name = 'N/A'
    if email is None or email.strip() == '':
        email = 'N/A'
    if skills is None or skills.strip() == '':
        skills = 'N/A'

    DB_table_name = 'user_data'
    insert_sql = f"INSERT INTO {DB_table_name} (Name, Email_ID, resume_score, Page_no, User_level, Actual_skills) VALUES (%s, %s, %s, %s, %s, %s)"
    rec_values = (name, email, res_score, no_of_pages, cand_level, skills)
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                if 'CERTIFICATION' or 'Certification' in resume_text:
                    resume_score = resume_score + 20
                    st.markdown(
                        '''<h4 style='text-align: left; color: #1ed760;'>[+] Awesome! You have completed few certification✍/h4>''',
                        unsafe_allow_html=True)
                else:
                    st.markdown(
                        '''<h4 style='text-align: left; color: #fabc10;'>[-] According to our recommendation please add few certification✍. It will give the assurance that you have some real time knowlegde</h4>''',
                        unsafe_allow_html=True)
                ...
                if resume_data['name'] and resume_data['email'] and resume_data['skills']:
                    ...
                    print("Error: One or more required fields are missing in the resume data.")
        ...
        if st.button('Login'):
            if ad_user == 'qwert123' and ad_password == '147258369':
                st.success("Welcome Admin")
                # Display Data
                cursor.execute('''SELECT*FROM user_data''')
                data = cursor.fetchall()
                st.header("**User's👨‍💻 Data**")
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Resume Score', 'Total Page', 'User Level', 'Actual Skills'])
                st.dataframe(df)
